...
public class DeviceServiceClient {
    ...
    public DeviceServiceClient(String baseUri, String productKey) {
        client = ClientBuilder.newClient();
        String resourcePath = MessageFormat.format("product/{0}/device", new Object[]{productKey});
        webTarget = client.target(baseUri).path(resourcePath);
    }
    
    public void setResourcePath(String baseUri, String productKey) {
        String resourcePath = MessageFormat.format("product/{0}/device", new Object[]{productKey});
        webTarget = client.target(baseUri).path(resourcePath);
    }
